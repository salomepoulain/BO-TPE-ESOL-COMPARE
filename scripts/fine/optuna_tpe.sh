#!/bin/bash
# Fine-pass TPE arm. Array task k runs seed (69+k) — same offset as the
# coarse pass so coarse vs fine is a paired comparison at each seed.
# All HPO storage on local SSD ($TMPDIR), rsynced to
# output/simulation/fine_tpe/<run>/ at the end.
# Search space is narrowed per report/fine_pass_plan.md (--search-space fine).
#SBATCH --job-name=ml4chem-fine-tpe
...
RUN_NAME="optuna_tpe_a${SLURM_ARRAY_TASK_ID}"
OUTPUT_DIR="output/simulation/fine_tpe"
...
uv run python -u src/run_optuna.py \
    --seed "$SEED" \
    --run-name "$RUN_NAME" \
    --output-dir "$TMPDIR" \
    --data-root data \
