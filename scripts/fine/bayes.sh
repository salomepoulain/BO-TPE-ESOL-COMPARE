#!/bin/bash
# Fine-pass BoTorch BO arm. Array task k runs seed (69+k) — same offset as
# the coarse pass so coarse vs fine is a paired comparison at each seed.
# All HPO storage on local SSD ($TMPDIR), rsynced to
# output/simulation/fine_bo/<run>/ at the end.
# Sampler internals are unchanged from coarse pass (LogExpectedImprovement,
# SingleTaskGP, q=1). Search space is narrowed per report/fine_pass_plan.md
# (--search-space fine).
#SBATCH --job-name=ml4chem-fine-bayes
...
RUN_NAME="bayes_a${SLURM_ARRAY_TASK_ID}"
OUTPUT_DIR="output/simulation/fine_bo"
